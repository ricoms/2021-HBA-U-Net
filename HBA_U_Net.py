...
class HBA(MultiHeadAttention):
    def __init__(self, num_heads=4, bottleneck_dimension=512, relative=True, **kwargs):
        ...
        self.num_heads, self.bottleneck_dimension, self.relative = (
            num_heads,
    ...
    def relative_logits(self, query):
        ...
        rel_logits_w = self.relative_logits_1d(
            query=query, rel_k=self.rel_emb_w, transpose_mask=[0, 1, 2, 4, 3, 5]
        )
        query = tf.transpose(query, [0, 1, 3, 2, 4])
        rel_logits_h = self.relative_logits_1d(
            query=query, rel_k=self.rel_emb_h, transpose_mask=[0, 1, 4, 2, 5, 3]
        )
    ...
    def call(
        ...
        attention_output = tf.reshape(
            attention_output, [-1, hh, ww, self.num_heads * self.key_dim]
        )
...
def HBA_block(
    ...
    if strides != 1 or featuremap.shape[-1] != target_dimension:
        shortcut = conv2d_no_bias(
            featuremap, target_dimension, 1, strides=strides, name=name + "_0_"
        )
        shortcut = batchnorm_with_activation(
            shortcut, activation=activation, zero_gamma=False, name=name + "_0_"
        )
    else:
        shortcut = featuremap
    ...
    nn = conv2d_no_bias(
        featuremap, bottleneck_dimension, 1, strides=1, name=name + "_1_"
    )
    nn = batchnorm_with_activation(
        nn, activation=activation, zero_gamma=False, name=name + "_1_"
    )
    ...
    if strides != 1:
        nn = layers.AveragePooling2D(pool_size=(2, 2), strides=(2, 2), padding="same")(
            nn
        )
    nn = batchnorm_with_activation(
        nn, activation=activation, zero_gamma=False, name=name + "_2_"
    )

    nn = conv2d_no_bias(nn, target_dimension, 1, strides=1, name=name + "_3_")
    nn = batchnorm_with_activation(
        nn, activation=None, zero_gamma=True, name=name + "_3_"
    )
...
def HBA_skip(
    ...
    nn = layers.AveragePooling2D(pool_size=strides, strides=strides, padding="same")(
        featuremap
    )
    nn = HBA(
        num_heads=heads, bottleneck_dimension=target_dimension, name=name + "_mhsa"
    )(nn)
    nn = batchnorm_with_activation(
        nn, activation=None, zero_gamma=True, name=name + "_3_"
    )
...
def HBA_U_Net(
    input_shape, dropout_rate=0.4, use_attnDecoder=False, skip=False, num_layers=3
):
    resnet_base = Encoder(
        include_top=True, classes=2, input_shape=(512, 512, 3), num_layers=num_layers
    )
